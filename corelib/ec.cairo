...
fn ec_add(p: Option::<EcPoint>, q: Option::<EcPoint>) -> Option::<EcPoint> {
    let p_nz = match p {
        Option::Some(pt) => pt,
        Option::None(()) => { return q; }
    };
    let q_nz = match q {
        Option::Some(pt) => pt,
...
fn ec_sub(p: Option::<EcPoint>, q: Option::<EcPoint>) -> Option::<EcPoint> {
    let q_nz = match q {
        Option::Some(pt) => pt,
        Option::None(()) => {
            // p - 0 = p.
    ...
    // p - q = p + (-q).
    ec_add(p, Option::Some(ec_neg(q_nz)))
}
