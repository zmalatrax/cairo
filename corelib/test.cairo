...
fn test_ec_operations() {
    ...
    let (double_x, double_y) = ec_point_unwrap(option_unwrap(double_p));
    ...
    // Compute `2p - p`.
    let (sub_x, sub_y) = ec_point_unwrap(option_unwrap(ec_sub(double_p, Option::Some(p))));
    assert(sub_x == x, 'bad x for 2p - p');
    assert(sub_y == y, 'bad y for 2p - p');

    // Compute `p - p`.
    assert(option_is_none(ec_sub(Option::Some(p), Option::Some(p))), 'p - p did not return 0.');

    // Compute `(-p) - p`.
    let (sub2_x, sub2_y) = ec_point_unwrap(
        option_unwrap(ec_sub(Option::Some(ec_neg(p)), Option::Some(p)))
    );
...
fn test_ecdsa() {
    assert(
        ecdsa::check_ecdsa_signature(
            message_hash: 0x503f4bea29baee10b22a7f10bdc82dda071c977c1f25b8f3973d34e6b03b2c,
            public_key: 0x7b7454acbe7845da996377f85eb0892044d75ae95d04d3325a391951f35d2ec,
            signature_r: 0xbe96d72eb4f94078192c2e84d5230cde2a70f4b45c8797e2c907acff5060bb,
            signature_s: 0x677ae6bba6daf00d2631fab14c8acf24be6579f9d9e98f67aa7f2770e57a1f5,
        ),
        'ecdsa failed'
    );
}
...
fn test_ec_mul() {
    ...
    let (x, y) = ec_point_unwrap(ec_mul(p, m));

    assert(
        x == 1729273150894356090937705498456562650283377825070249315957175834940134748759,
        'ec_mul failed (x).'
    );
    assert(
        y == 2497033140637529885552528606019014506601504662967221800282254926345878420830,
        'ec_mul failed (y).'
    );
}
