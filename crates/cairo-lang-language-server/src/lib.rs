...
impl LanguageServer for Backend {
    ...
    async fn did_open(&self, params: DidOpenTextDocumentParams) {
        ...
        db.override_file_content(file, Some(Arc::new(params.text_document.text.into())));
        drop(db);
    ...
    async fn completion(&self, params: CompletionParams) -> LSPResult<Option<CompletionResponse>> {
        self.with_db(|db| {
            ...
            let Some((mut node, lookup_items)) =
...
fn completion_kind(db: &RootDatabase, node: SyntaxNode) -> CompletionKind {
    ...
    match node.kind(db) {
        ...
        SyntaxKind::TerminalColonColon => {
            ...
            let Ok(()) = get_use_segments(
                    db.upcast(), &use_ast, &mut segments
                ) else {
                    eprintln!("Generic");
                    return CompletionKind::ColonColon(vec![]);
                };
        ...
        SyntaxKind::TerminalIdentifier => {
            ...
            if grandparent.kind(db) == SyntaxKind::UsePathLeaf {
                ...
                let mut segments = vec![];
                let Ok(()) = get_use_segments(
                    db.upcast(), &ast::UsePath::Leaf(use_ast), &mut segments
                ) else {
                    eprintln!("Generic");
                    return CompletionKind::ColonColon(vec![]);
                };
