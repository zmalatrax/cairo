...
pub fn colon_colon_completions(
    ...
    eprintln!("colon_colon_completions");
    ...
    eprintln!(
        ...
        segments
            .iter()
            .map(|s| s.as_syntax_node().get_text_without_trivia(db.upcast()))
            .collect::<Vec<_>>()
    );
    let item = resolver
...
pub fn dot_completions(
    ...
    for trait_function in relevant_methods {
        let Some(completion) = completion_for_method(db, module_id, trait_function, position) else {
            continue;
        };
